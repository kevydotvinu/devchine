...
resize-disk:
	@sudo virsh destroy metallb || true && sudo qemu-img resize ${METALLB_DIR}/metallb.qcow2 18G && sudo virsh start metallb

.PHONY: ssh-dnat

ssh-dnat: check-metallb-ip
	@sudo iptables-save | grep -q '\--dport 2222' && sudo iptables -t nat $$(sudo iptables-save | grep '\--dport 2222' | sed 's/^-A/-D/') || true
